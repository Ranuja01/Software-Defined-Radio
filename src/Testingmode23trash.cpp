...
void impulseResponseLPF2(float Fs, float Fc, unsigned short int num_taps, const int &gain, std::vector<float> &h)
{
...
void convolveFIR2(std::vector<float> &y, const std::vector<float> &x, const std::vector<float> &h, std::vector<float> &zi, const int &down, const int &up)
{
		// allocate memory for the impulse response
		y.clear();
...
int main()
{
 ...
 // std::vector<uint8_t> iq_data;
  ...
  if(mode == 0){
     ...
     audio_upSample = 147;
     audio_decim = 800;
 		blockSize = 1029*rf_decim * ((float)audio_decim/audio_upSample) * 2;
